...
float Nucleus::scale_attraction(float attraction, const shared_ptr<const Particle>& other, const RelativePosition& rel_pos)
{
	if (other->is_type(ParticleType::Food))
	{
		return attraction * _hunger_scaling * (1.0f - get_health_percent());
	}
	if (other->is_type(ParticleType::Nucleus))
	{
...
bool Nucleus::ignore_attraction(const shared_ptr<const Particle>& other, const RelativePosition& rel_pos) const
{
	if (other->is_type(ParticleType::Nucleus))
	{
...
void Nucleus::write_unique_data(ofstream& stream) const
{
	FileUtils::write_int32(stream, _current_food);
}

void Nucleus::read_unique_data(ifstream& stream)
{
